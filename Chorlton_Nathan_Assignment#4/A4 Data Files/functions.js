...
// shows date as a string containing the week day, month, day of the month, and the year
function showDate() {
   //array to find the weekday
   const dayNames = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", ];

   //array to find the name of the month
   const monthNames = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
   
   //creates a varible containing the current day
   var currentDay = new Date();

   /* display the current date and time in the desired format*/
   document.getElementById("date").innerHTML = dayNames[currentDay.getDay()] + ", " + monthNames[currentDay.getMonth()] + " " + currentDay.getDate() + ", " + currentDay.getFullYear();
}

//generates the day out of 366 so it can pass it to the showBirthDay() function 
function dayNumber() {
   
   //creates a varible with the current date
   var today = new Date();

   //gets the first day of the current year
   var start = new Date(today.getFullYear(), 0, 0);

   //find the millisecconds its been since the start of the year
   var difference = today - start;

   //variable to hold the milisecconds per day
   var milisecondsPerDay = (1000 * 60 * 60 * 24);

   //uses the variable above the ammount of days since the begenning of the day, and turns into a number out of 366
   var dayOutOf366 = Math.floor(difference / milisecondsPerDay);

   //returns this number so it can be used in the births array
   return Math.floor(dayOutOf366);
}

//takes the day out of 366, applies it to the births array
function showBirthDay(day) {
   document.getElementById("dayOfTheYear").innerHTML =  births[day];
}
...
var births = new Array();
   ...
   var dayToday = dayNumber();

   showBirthDay(dayToday);
